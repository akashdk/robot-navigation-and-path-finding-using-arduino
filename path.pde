...
int a=600,b=400;
int maxSamples = 400;
float comp,curx,cury,curz,prevx,prevy,prevz,xa,ya,za;
int sampleStep = 1;
int temp=1;
...
void drawCompass(int x, int y, float[][] head, PImage plate)
{
  ...
  image(bat,1000,30,x/3,y/4);
  image(BT,1000,230,x/3,y/4);
  image(metal,1000,400,x/3,y/4);
...
//  drawCompass(480, 5, headingValues, imgCompassPlateWhite);
  drawCompass(480, 500, headingValues, imgCompassPlateBlack);
//print(comp);
//println("");
//magneticValues[0][actualSample]
...
if((comp>260)&&(comp<280))
{
  a++;
}
if((comp>170)&&(comp<190))
{
  b++;
}
if((comp>10)&&(comp<70))
...
if((comp>100)&&(comp<160))
{
  a--;
  b++;
}
if((comp>190)&&(comp<250))
{
  a++;
  b++;
}
if((comp>280)&&(comp<350))
{
  a++;
  b--;
}
...
void serialEvent (Serial myPort)
{
  ...
  if (inString != null)
  {
    ...
    curx =  magneticValues[0][actualSample];
    cury =  magneticValues[1][actualSample];
    curz =  magneticValues[2][actualSample];
    comp = headingValues[2][actualSample];
